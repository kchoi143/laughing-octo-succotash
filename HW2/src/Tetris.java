...
public class Tetris extends JPanel { // panel in frame to add objects
	...
	public Tetris() {
		...
		f.add(this);
		f.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); // terminate application when X is closed
		f.setSize(400, 550);
		f.setVisible(true); 
		EventController ec = new EventController(game);
		f.addKeyListener(ec); // notified by frame with any key pressed
